...
const Dashboard = () => {
  return (
    <div className=" text-white   max-sm:py-4 max-sm:px-2     container w-full   ">
      {/* My Projects Section */}
      <div className="bg-white bg-opacity-5  border border-customBlue  max-sm:h-[690px]  mx-auto  top-[139px] gap-[24px] rounded-[24px] border-t-[1px]    h-[540px]   ">
        ...
        <div className="space-y-4 py-8  p-6  ">
          {data.map((item) => (
            <div
              key={item.id}
              className="w-full h-fit max-sm:h-[153px] pt-4 max-sm:p-3 gap-4 rounded-[16px] border flex flex-wrap max-sm:block max-sm:space-y-4 "
            >
              <img
                src={item.logo}
                alt="Logo"
                className="w-10 h-10 gap-0 rounded-[10px] border mb-3 max-sm:mb-0 ml-2 max-sm:ml-0"
              />

              <div className="text-base font-normal leading-[19.2px] text-left top-8 max-sm:space-y-2 ">
                <div>{item.title}</div>
                <div className=" lg:hidden max-sm:space-y-4">
                ...
                <div className="max-sm:flex max-sm:items-center max-sm:gap-12">
                  <div className="w-[74px] bg-gray-200 rounded h-1   sm:hidden">
                    <div
                      className="bg-gray-100 h-1 rounded"
                      style={{
                        width: `${
                  ...
                  <span className="sm:hidden text-[#B3B8DB] text-[12px]">{`${
                    item.progress?.completed || 0
                  }/${item.progress?.total || 0} Tasks Completed`}</span>
                </div>
              </div>
